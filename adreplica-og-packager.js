...
function buildAppMarkSvg() {
  return [
    ...
    '  <path d="M63.4 53.4 72.6 67h-8.4l-5.9-13.6Z" fill="url(#adreplica-gold)"/>',
    '</svg>',
  ].join("\n");
}
...
function buildLandingHtml({ appName, build, bookmarklet, manifestOgObjectId, screenshotUrl, iconUrl }) {
  ...
  return [
    ...
    "          <div class=\"install\">",
    `            <button class="bookmarklet" id="copyBookmarklet" type="button" data-bookmarklet="${escapeHtml(bookmarklet)}">AdReplica</button>`,
    "            <p class=\"hint\">Click the yellow button to copy the loader bookmarklet. Then create a bookmark in your browser and paste it into the URL field.</p>",
    "          </div>",
    "          <div class=\"actions\">",
    ...
    "      <div class=\"step\"><b>1. Install</b>Click the AdReplica button to copy the loader, then paste it into a new bookmark URL.</div>",
