(function adReplicaBootstrap() {
  ...
  const state = {
    ...
    importTargetBusiness: null,
    accountCampaignsCache: {},
    accountContextCache: {},
    accessiblePagesCache: null,
    exportAccountId: "",
    importAccountId: "",
    exportCampaigns: [],
    exportCampaignId: "",
    importPackage: null,
    importMediaFiles: new Map(),
    importMediaOverrides: new Map(),
    importPageMappings: {},
    importPixelMappings: {},
    importCatalogMappings: {},
    importCampaignName: "",
    importAsDraft: true,
    importStatus: "PAUSED",
    importPreserveSchedule: false,
    cloneSourceAccountId: "",
    cloneSourceCampaignId: "",
    cloneSourceCampaigns: [],
    cloneTargetAccountId: "",
    clonePackage: null,
    clonePackageLoading: false,
    clonePages: [],
    cloneTargetPixels: [],
    cloneSourceCatalogs: [],
    cloneSourceBusiness: null,
    cloneTargetCatalogs: [],
    cloneTargetBusiness: null,
    clonePageMappings: {},
    clonePixelMappings: {},
    cloneCatalogMappings: {},
    cloneManualSourceCatalogId: "",
    cloneCampaignName: "",
    cloneAsDraft: true,
    cloneStatus: "PAUSED",
    pageIdentityHints: null,
    lastDraftIdentityRepair: null,
    graphTemplateAccountId: "",
    graphTemplateParams: null,
    tempIdCursor: null,
    reloadTimer: null,
    uiReady: false,
  };
  ...
  function buildAdReplicaLoaderSource(loaderConfig) {
  ...
  function log(level, message, details) {
    ...
    if (level === "error") {
      console.error(`[${APP_TITLE}] ${message}`, details ?? "");
    } else if (level === "warn") {
      console.warn(`[${APP_TITLE}] ${message}`, details ?? "");
    } else {
      console.log(`[${APP_TITLE}] ${message}`, details ?? "");
    }
    renderLogs();
    return entry;
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      #${APP_ID} .sk-byline:hover { opacity: 1; text-decoration: underline; }
      #${APP_ID} .sk-actions,
      #${APP_ID} .sk-row,
      #${APP_ID} .sk-log-head {
        display: flex;
      ...
      #${APP_ID} .sk-tab.sk-tab-active {
        background: #1a1a1a !important;
        color: #ffc107 !important;
        border-bottom: 2px solid #1a1a1a !important;
      }
      #${APP_ID} .sk-tab-panel { margin-top: 14px; }
      #${APP_ID} .sk-hidden { display: none !important; }
      #${APP_ID} .sk-card,
      #${APP_ID} .sk-logs {
        background: #222;
      ...
      #${APP_ID} button:hover:not(:disabled) {
        background: #ffca28;
      }
      #${APP_ID} button.sk-secondary {
        background: #333;
        color: #ccc;
        border: 1px solid #555;
      }
      ...
      #${APP_ID} .sk-note,
      #${APP_ID} .sk-empty,
      #${APP_ID} .sk-creative-file,
      #${APP_ID} .sk-summary-grid span {
        color: #aaa;
        font-size: 13px;
      }
      #${APP_ID} .sk-clone-summary {
        margin: 8px 0 14px;
      }
      #${APP_ID} .sk-clone-summary strong {
        color: #f5f5f5;
      }
      #${APP_ID} .sk-warning {
        color: #f4c36b;
      ...
      #${APP_ID} .sk-check input {
        width: auto;
      }
      #${APP_ID} .sk-compact {
        align-items: end;
      }
      #${APP_ID} .sk-summary-grid {
        display: grid;
        grid-template-columns: repeat(3, minmax(0, 1fr));
        gap: 10px;
      }
      #${APP_ID} .sk-summary-grid div {
        background: #2a2a2a;
        border-radius: 8px;
        padding: 10px;
        display: grid;
      ...
      #${APP_ID} .sk-mapping-block + .sk-mapping-block {
        margin-top: 14px;
      }
      #${APP_ID} .sk-creative-row {
        display: grid;
        gap: 8px;
        padding: 10px;
        background: #2a2a2a;
      ...
      #${APP_ID} .sk-creative-row + .sk-creative-row {
        margin-top: 8px;
      }
      #${APP_ID} .sk-creative-info {
        display: grid;
        gap: 4px;
      }
      #${APP_ID} .sk-creative-name {
        display: flex;
        gap: 8px;
        align-items: center;
        font-weight: 600;
      ...
      #${APP_ID} .sk-badge-image {
        background: rgba(59, 130, 246, 0.25);
        color: #93c5fd;
      }
      #${APP_ID} .sk-badge-video {
      ...
      @media (max-width: 640px) {
        ...
        #${APP_ID} .sk-row { flex-direction: column; align-items: stretch; }
      }
    `;
  ...
  function getCurrentActorId() {
    return (document.cookie.match(/(?:^|;\s)c_user=(\d+)/) || [])[1]
      || (typeof require === "function" && require("CurrentUserInitialData")?.USER_ID)
      || "";
  }
  ...
  async function initializeSession(force = false) {
    ...
    try {
      ...
      const runtimeDtsg =
        typeof require === "function" && require("DTSGInitialData")?.token
          ? require("DTSGInitialData").token
          : (typeof require === "function" && require("DTSGInitData")?.token
            ? require("DTSGInitData").token
            : "");
      const runtimeLsd = typeof require === "function" && require("LSD")?.token
        ? require("LSD").token
        : "";
  ...
  async function loadAccounts() {
    ...
    const currentAccountId =
      typeof require === "function" && require("BusinessUnifiedNavigationContext")?.adAccountID
        ? String(require("BusinessUnifiedNavigationContext").adAccountID).replace(/^act_/, "")
        : "";
    if (!state.exportAccountId) {
